            ...
            <div id="slider-wrap">
                <div id="slider">
                	...
                	foreach($arResult["ITEMS"] as $arItem)
                	{
                		...
                		$fid = intval($arItem["PREVIEW_PICTURE"]["ID"]);
                		if($fid > 0)
                		{
                			$arPhotoFilter = array("name" => "sharpen", "precision" => 30);
                			$arFileTmp = CFile::ResizeImageGet($fid,array("width" => 949, "height" => 315),BX_RESIZE_IMAGE_EXACT,true, $arPhotoFilter);
                			if(!empty($arFileTmp["src"]))
                			{
                				?>
                				<div class="slide" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
			                        <img src="<?=$arFileTmp["src"]?>" alt="" width="949" height="315" />
			                        <div class="slide-title-wrap">
			                        	<h3 class="slide-title">
			                        		<?=$arItem["~PREVIEW_TEXT"]?>
			                        	</h3>
			                        	<?
			                        	$price = floatval($arItem["PROPERTIES"]["PRICE"]["VALUE"]);
			                        	if($price > 0)
			                        	{
			                        		$price_f = number_format($price,0,"."," ");
			                        		?>
			                        		<p class="slide-price"><strong><?=$price_f?></strong> руб.</p>
			                        		<?
			                        	}
			                        	?>
			                        	</div>
			                        ...
			                        {
			                        	?>
			                        	<a href="<?=$url?>" class="slide-more">Подробнее »</a>
			                        	<?
			                        }
			                        ?>
			                    </div>
