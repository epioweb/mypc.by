	...
	<table>
						...
						<div id="basic" class="svwp" style="float:left">
							<ul>
								...
								<?elseif(is_array($arResult["DETAIL_PICTURE"])):
									...
									if(!empty($arResult["DETAIL_PICTURE"]["ORIGINAL_SRC"]))
										$src = $arResult["DETAIL_PICTURE"]["ORIGINAL_SRC"];
									?>
									<li><a rel="catalog-detail-images" class="catalog-detail-images" href="<?=$src?>" title="<?=(strlen($arResult["DETAIL_PICTURE"]["DESCRIPTION"]) > 0 ? $arResult["DETAIL_PICTURE"]["DESCRIPTION"] : $arResult["NAME"])?>"><img width="280" itemprop="image" src="<?=$arResult["DETAIL_PICTURE"]["SRC"]?>" alt="<?=$arResult["NAME"]?>" title="<?=$arResult["NAME"]?>" /></a></li>
								...
								<?foreach($arResult["MORE_PHOTO"] as $PHOTO):
									$src = $PHOTO['SRC'];
									if(!empty($PHOTO["ORIGINAL_SRC"]))
										$src = $PHOTO["ORIGINAL_SRC"];
									?>
									<li><a rel="catalog-detail-images" class="catalog-detail-images" href="<?=$src?>" title="<?=(strlen($PHOTO["DESCRIPTION"]) > 0 ? $PHOTO["DESCRIPTION"] : $PHOTO["NAME"])?>"><img  src="<?=$PHOTO["SRC"]?>"  alt="<?=$arResult["NAME"]?>" title="<?=$arResult["NAME"]?>" /></a></li>
								<?endforeach?>
		...
		<?endif;?>
			<td class="iteminfo">
				...
				{
				?>
				<?    //  COption::SetOptionString("eshop", "catalogDetailSku", "select", false, SITE_ID);
					...
					<tr>
						...
						<td>
							<?if (intval($arSKU["DISCOUNT_PRICE"]) > 0 && $arSKU["PRICE"] > 0):?>
								<span class="discount-price"><?=$arSKU["DISCOUNT_PRICE"]?></span> <span class="old-price"><?=$arSKU["PRICE"]?></span>
							<?else:?>
								<?=$arSKU["PRICE"]?>
							<?endif?>
						</td>
