	...
	<li class="itembg R2D2" id="<?=$this->GetEditAreaId($arElement['ID']);?>">
			...
			<?if(is_array($arElement["PREVIEW_IMG"])):?>
				<table style="width:200px !important;height:180px !important;">
					<tr>
						<td class="tac vam" style="width: 200px !important;height:180px !important;">
							<a href="<?=$arElement["DETAIL_PAGE_URL"]?>"><img class="item_img" border="0" src="<?=$arElement["PREVIEW_IMG"]["SRC"]?>" alt="<?=$arElement["NAME"]?>" title="<?=$arElement["NAME"]?>" /></a>
						</td>
					</tr>
				</table>
			<?elseif(is_array($arElement["PREVIEW_PICTURE"])):?>
				<a href="<?=$arElement["DETAIL_PAGE_URL"]?>"><img class="item_img" border="0" src="<?=$arElement["PREVIEW_PICTURE"]["SRC"]?>" alt="<?=$arElement["NAME"]?>" title="<?=$arElement["NAME"]?>" /></a>
			<?else:?>
				<a href="<?=$arElement["DETAIL_PAGE_URL"]?>"><div class="no-photo-div-big" style="height:<?=$arParams["DISPLAY_IMG_HEIGHT"]?>px; width:<?=$arParams["DISPLAY_IMG_WIDTH"]?>px; margin:auto;"></div></a>
			<?endif?>
			<hr/>
