...
foreach($arResult["ITEMS"] as $cell=>$arElement)
{
	...
	if($sid > 0)
	{
		$section = $sectionsCache[$sid];
		if(!is_array($section))
		{
			$section = CIBlockSection::GetByID($sid)->fetch();
		}
		
		if(is_array($section))
		{
			$sectionsCache[$section["ID"]] = $section;
			$rs = CIBlockSection::GetNavChain($arElement["IBLOCK_ID"],$sid);
			while($sec = $rs->fetch())
			{
				$sectionChainUrl .= $sec["CODE"]."/";
			}
		}
	}
	
	//$arResult["ITEMS"][$cell]["DETAIL_PAGE_URL"] = $arParams["CATALOG_SEF_FOLDER"].$sectionChainUrl.$arElement["CODE"].".html";
	$arResult["ITEMS"][$cell]["DETAIL_PAGE_URL"] = $arParams["CATALOG_SEF_FOLDER"].$sectionChainUrl.$arElement["PROPERTIES"]["ARTNUMBER"]["VALUE"].".html";
...
foreach ($arResult["ITEMS"] as $key => $arElement) 
{
	...
	if(is_array($photos) && count($photos) > 0 && !is_array($arResult["ITEMS"][$key]["PREVIEW_IMG"]))
	{
		$fid = reset($photos);
		$fid = intval($fid);
		if($fid > 0)
